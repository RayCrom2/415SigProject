 ...
 *    srand, rand    the traditional library functions for random number generation       *
 *                                                                                       *
 * to compile and run:                                                                   *
...
#include "first-names.c"    // 2000 first names, common in the United States
#include "surnames.c"       // 1000 surnames, common in the United States
#include "states.c"         // names of 57 U. S. states and territories
...
const int numSimIterations = 20000;
...
void sortAlgA(std::vector<Person*>& vec, bool(comesBefore(Person, Person)), bool(equals(Person, Person))) {
...
void sortAlgBHelper(std::vector<Person*>& vec, const int& smallIndex, const int& i, int& j){
...
void sortAlgB(std::vector<Person*>& vec,
    ...
    int i, j, smallIndex;
        ...
        for (i = int(vec.size() - 1); i >= 0; i--) {
            ...
            // insert max element into last position by sliding other elements down
            sortAlgBHelper(vec, smallIndex, i, j);
        }
}
/** **************************************************************************************
